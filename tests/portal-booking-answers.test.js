...
const goodAnswers = (service = svc) => [
  { serviceId: service.id, questionId: idOf(service, 'What is wrong?'), text: 'Rubs at the back' },
  { serviceId: service.id, questionId: idOf(service, 'Tubeless?'), choice: 'No' },
];
...
test('not sure is stored as { notSure: true }', async () => {
  const res = await book({ answers: [...goodAnswers(), { serviceId: svc.id, questionId: idOf(svc, 'E-bike?'), notSure: true }] });
  assert.equal(res.status, 201, JSON.stringify(res.body));
...
test('rewording and then deleting the questions leaves the booking unchanged', async () => {
  ...
  const res = await book({ serviceIds: [own.id], answers: goodAnswers(own) });
  assert.equal(res.status, 201, JSON.stringify(res.body));
...
test('a service with no questions stores an empty copy', async () => {
  ...
  const res = await book({ serviceIds: [plain.id] });
  assert.equal(res.status, 201, JSON.stringify(res.body));
  assert.deepEqual(await stored(res.body.id), []);
});
...
test('a choice not on the list is refused', () =>
  refusedAsGuest({ answers: [goodAnswers()[0], { serviceId: svc.id, questionId: idOf(svc, 'Tubeless?'), choice: 'Maybe' }] }, /have changed/));
test('not sure where it is switched off is refused', () =>
  refusedAsGuest({ answers: [goodAnswers()[0], { serviceId: svc.id, questionId: idOf(svc, 'Tubeless?'), notSure: true }] }, /have changed/));
test('a text answer over 1,000 characters is refused', () =>
  refusedAsGuest({ answers: [{ serviceId: svc.id, questionId: idOf(svc, 'What is wrong?'), text: 'x'.repeat(1001) }, goodAnswers()[1]] }, /1,000 characters/));
test('an unknown question id is refused with the changed message', () =>
  refusedAsGuest({ answers: [...goodAnswers(), { serviceId: svc.id, questionId: 'q_gone00000000', text: 'x' }] },
...
test('answers on a not sure booking are refused', () =>
  refusedAsGuest({ serviceIds: undefined, notSure: true, answers: goodAnswers() }, /chosen service/));
test('a not sure booking with non-list answers is refused', () =>
  refusedAsGuest({ serviceIds: undefined, notSure: true, answers: {} }, /chosen service/));
