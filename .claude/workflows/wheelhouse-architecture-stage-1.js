...
FIRST, work out which checks this branch actually has. The frontend scaffold
(Vite/React/Tailwind/shadcn) may or may not be merged into master yet - if PR #9
is unmerged, the build, lint, typecheck and registry scripts will not exist here.
Run \`node -e "console.log(Object.keys(require('./package.json').scripts).join(' '))"\`
and \`ls scripts/ci/ 2>/dev/null\` and decide from that.

Run and record REAL output for everything that exists:
  npm test                                  (always - exact pass/fail totals)
  npm run migrate  (twice - CI asserts idempotency)
  node scripts/ci/assert-rls-coverage.mjs   (if present)
  npm run typecheck / lint / build          (if those scripts exist)
  npm run registry:validate                 (if present)
  node scripts/ci/check-registry-drift.mjs  (if present)

For anything absent, say so explicitly and record it as not-run rather than
silently omitting it or marking it passed. An absent check is not a passing
check. State plainly in your report which gates existed on this branch.
...
Assess honestly:
  ...
  - Note whether the frontend scaffold (PR #9) is present on this branch. If it
    is not, several CI gates do not exist here and the verifier could not run
